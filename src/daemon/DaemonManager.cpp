...
bool DaemonManager::start(const QString &flags, NetworkType::Type nettype, const QString &dataDir, const QString &bootstrapNodeAddress)
{
    ...
    qDebug() << "starting mazed " + m_monerod;
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM mazed.exe");
#else
                QProcess::execute("pkill mazed");
#endif
            }
...
#ifdef Q_OS_WIN
    m_monerod = QApplication::applicationDirPath() + "/mazed.exe";
#elif defined(Q_OS_UNIX)
    m_monerod = QApplication::applicationDirPath() + "/mazed";
#endif
